...
Provision the single AWS Lambda function that runs the entire simulation engine as a container image (per the project's hard constraint: "simulation runs inside a single AWS Lambda container image"), expose it via a Function URL that Supabase's Database Webhook will call, and wire minimal IAM permissions to the DynamoDB tables (task 001) and S3 buckets (task 002). Reference: README.md "Hard Constraints" (b: trigger is Supabase Database Webhook → Lambda Function URL) and CLAUDE.md Stack section.
...
6. A Function URL MUST be created with `authType: lambda.FunctionUrlAuthType.NONE` initially (Supabase webhooks call it with a shared-secret header validated at the application layer, not via AWS IAM auth) — document this as a technical note, not a requirement gap.
7. The Function URL's CORS configuration MUST allow no origins (`allowedOrigins: []`) since it is called server-to-server by Supabase, never from a browser.
